    ...
    if (result.success && result.job) {
      ...
      return {
        ...
        openGraph: {
          ...
          description: description,
          type: "website",
          url: `${process.env.NEXT_PUBLIC_APP_URL || 'https://jobkarle.com'}/candidate/jobs/${jobId}`,
          siteName: "JobKarle",
        },
        twitter: {
...
export default async function JobDetailPage({ params }: { params: Promise<{ jobId: string }> | { jobId: string } }) {
  try {
    console.log("[v0] Job Details Page: Starting to load")

    const resolvedParams = await Promise.resolve(params)
    const { jobId } = resolvedParams

    console.log("[v0] Job Details Page: Job ID:", jobId)
    console.log("[v0] Job Details Page: Full URL params:", resolvedParams)

    if (!jobId || typeof jobId !== "string" || jobId.trim().length === 0) {
      console.log("[v0] Job Details Page: Invalid or missing job ID")
      return <JobNotFoundError message="Invalid job ID format" jobId={jobId || "N/A"} />
    }

    console.log("[v0] Job Details Page: Getting candidate session")
    const { success, session } = await getCandidateSession()
    console.log("[v0] Job Details Page: Session result:", { success, candidateId: session?.candidateId })

    console.log("[v0] Job Details Page: This page is PUBLIC - no authentication required to view job details")

    console.log("[v0] Job Details Page: Fetching job details for job:", jobId)
    const result = await getJobDetails(jobId, session?.candidateId)
    console.log("[v0] Job Details Page: Job details result:", {
    ...
    if (!result.success || !result.job) {
      console.error("[v0] Job Details Page: Job not found or fetch failed:", {
        success: result.success,
        error: result.error,
        jobId,
      })
    ...
    if (success && session?.candidateId) {
      console.log("[v0] Job Details Page: Checking if job is saved")
      const savedResult = await isJobSaved(session.candidateId, jobId)
      isSaved = savedResult.isSaved || false
      console.log("[v0] Job Details Page: Is saved:", isSaved)
    }

    console.log("[v0] Job Details Page: Rendering JobDetailView component")
    console.log("[v0] Job Details Page: User can view all details. Login only required for Apply/Save actions")
    ...
    console.error("[v0] Job Details Page: ERROR occurred:", error)
    console.error("[v0] Job Details Page: Error details:", {
